...
async def get_all_notes(session: AsyncSession = Depends(get_session),token_details:dict=Depends(access_token_bearer))->dict:
    
    return await note_service.get_all_notes(session)


@note_router.get("/{note_id}", response_model=NoteResponseModel)
async def get_note_by_id(note_id:int,session:AsyncSession = Depends(get_session),token_details:dict=Depends(access_token_bearer)):
...
async def create_note(note_data:NoteCreateModel,session:AsyncSession = Depends(get_session),token_details:dict=Depends(access_token_bearer)):

    return await note_service.create_note(note_data,session)


@note_router.patch("/{note_id}",response_model=NoteResponseModel)
async def update_note(note_id:int,note_update_data:NoteUpdateModel,session:AsyncSession = Depends(get_session),token_details:dict=Depends(access_token_bearer)):
...
async def delete_a_note(note_id:int,session:AsyncSession = Depends(get_session),token_details:dict=Depends(access_token_bearer)):
