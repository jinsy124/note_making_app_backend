...
class NoteService:
    async def get_all_notes(self, session: AsyncSession) -> List[NoteResponseModel]:
        result = await session.execute(select(NoteModel).order_by(NoteModel.created_at.desc()))
    ...
    async def get_note_by_id(self,note_id:int,session: AsyncSession):
    ...
    async def create_note(self, note_data: NoteCreateModel, session: AsyncSession):
        note_data_dict = note_data.model_dump()
        new_note = NoteModel(**note_data_dict)
    ...
    async def update_note(self, note_id:int, note_update_data: NoteUpdateModel, session: AsyncSession):
        note = await session.get(NoteModel, note_id)
        if not note:
            return None
